...
class NetworkMonitor:
    def __init__(self, root):
        ...
        model_path = os.path.join(os.path.dirname(__file__), 'scapy_model.pkl')
    ...
    def analyze_packet(self, packet):
        ...
        if features is not None:
            ...
            # Color-coded log message
            if attack_prob > 0.7:
                log_color = "#ff6b6b"  # Red for high probability attack
            else:
                log_color = "#69db7c"  # Green for normal/low probability
            
            log_message = f"[{timestamp}] {protocol} {src_ip} → {dst_ip} Flags: {flags} : {result} (Attack prob: {attack_prob:.3f})\n"
            
            self.log_area.tag_config(f"color_{self.packet_count}", foreground=log_color)
